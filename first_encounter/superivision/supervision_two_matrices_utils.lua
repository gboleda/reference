...
local function add_new_input_and_create_double_mapping(inputs, input_size, mapping_size, dropout_p, share_mappings_select, 
   ...
   mapping_select = nn.ReLU()(mapping_select)
   mapping_compare = nn.ReLU()(mapping_compare)
...
local function build_entity_libary_2matrices(t_inp_size, v_inp_size, mm_size, inp_seq_cardinality, dropout_p, inputs, 
   ...
   local entity_creation_prob_table = {}
   ...
   for i=2,inp_seq_cardinality do
      ...
      local normalized_similarity_profile, transformed_new_entity_mass = weight_distribution_function(raw_similarity_profile_to_entity_matrix_select, i, temperature, raw_new_entity_mass_mappings)
      table.insert(entity_creation_prob_table, transformed_new_entity_mass)
   ...
   return entity_matrix_table_select, entity_matrix_table_compare, entity_creation_prob_table

end


local function build_entity_libary_2matrices_norm(t_inp_size, v_inp_size, mm_size, inp_seq_cardinality, dropout_p, inputs, 
                             attribute_mappings_select, attribute_mappings_compare, token_object_mappings_select, 
                             token_object_mappings_compare, raw_new_entity_mass_mappings,
                             temperature, weight_distribution_function)
   -- now we process the object tokens
   ...
   for i=2,inp_seq_cardinality do
      ...
      if (i ~= inp_seq_cardinality) then
         local weighted_object_token_vector_matrix_select = nn.MM(false,true){nn.View(-1,1):setNumInputDims(1)(normalized_similarity_profile),object_token_vector_select}
         local updated_entity_matrix_select = nn.CAddTable(){
            nn.Padding(1,1,2)(entity_matrix_table_select[i-1]),weighted_object_token_vector_matrix_select}:annotate{'entity_matrix_table_select' .. i}
         entity_matrix_table_select[i]= nn.Normalization()(updated_entity_matrix_select)
      end
      local weighted_object_token_vector_matrix_compare = nn.MM(false,true){nn.View(-1,1):setNumInputDims(1)(normalized_similarity_profile),object_token_vector_compare}
      local updated_entity_matrix_compare = nn.CAddTable(){
         nn.Padding(1,1,2)(entity_matrix_table_compare[i-1]),weighted_object_token_vector_matrix_compare}:annotate{'entity_matrix_table_compare' .. i}
       entity_matrix_table_compare[i] = nn.Normalization()(updated_entity_matrix_compare)
   end
...
function build_customize_model_with_2matrices(t_inp_size,v_inp_size,mm_size,inp_seq_cardinality, weight_distribution_function, temperature, dropout_p,use_cuda)
   ...
   local entity_matrix_table_select, entity_matrix_table_compare, normalized_weight_table_and_output_distribution = build_entity_libary_2matrices(t_inp_size, v_inp_size, mm_size, inp_seq_cardinality, dropout_p, inputs, 
                             attribute_mappings_select, attribute_mappings_compare, token_object_mappings_select, 
   ...
   table.insert(normalized_weight_table_and_output_distribution, output_distribution)
   -- wrapping up the model
   local model = nn.gModule(inputs,normalized_weight_table_and_output_distribution)
