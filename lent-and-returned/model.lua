...
function entity_prediction(t_inp_size,v_inp_size,mm_size,inp_seq_cardinality)
   ...
   -- also sharing the same multimodal word mapping matrix
   query_object.data.module:share(query_attribute_1.data.module,'weight','gradWeight')
   ...
   -- sharing the weights with the word mappings above
   first_token_attribute.data.module:share(query_attribute_1.data.module,'weight','gradWeight')
   ...
   for i=2,inp_seq_cardinality do
      ...
      -- sharing the word mapping weights
      token_attribute.data.module:share(query_attribute_1.data.module,'weight','gradWeight')
...
-------------
-------------
--  version with no weight sharing between query and candidates
function entity_prediction_query_token_mappings(t_inp_size,v_inp_size,mm_size,inp_seq_cardinality)
...
----- DELIBERATELY BUGGY FROM HERE

-------------
-------------
function entity_prediction_buggy(t_inp_size,v_inp_size,mm_size,inp_seq_cardinality)
   ...
   table.insert(inputs,curr_input)
   local query_object = nn.LinearNB(t_inp_size, mm_size)(curr_input):annotate{name='query_object'}
   -- sharing multimodal word mapping matrix of att1
   query_object.data.module:share(query_attribute_1.data.module,'weight','gradWeight')
...
----- DELIBERATELY BUGGY TO HERE

-- a version of our model that uses separate mappings for words
-- depending on whether they are query attributes, query objects or
-- candata attributes
-- NB: we might stop updating this model with other changes we might
-- make to the main, so it might soon become obsolete

function entity_prediction_separate_mappings(t_inp_size,o_inp_size,mm_size,inp_seq_cardinality)
...
------- WRONG SHARING FOR OLD MODEL FROM HERE
-- a version of our model that uses separate mappings for words
-- depending on whether they are query attributes, query objects or
-- candata attributes
-- NB: we might stop updating this model with other changes we might
...
function entity_prediction_old_wrong_sharing(t_inp_size,o_inp_size,mm_size,inp_seq_cardinality)
   ...
   table.insert(inputs,curr_input)
   local query_object = nn.LinearNB(t_inp_size, mm_size)(curr_input):annotate{name='query_object'}
   -- deliberately sharing matrix with first attribute (no bias/gradBias sharing
   -- since we're not using the bias term)
   query_object.data.module:share(query_attribute_1.data.module,'weight','gradWeight')
...
------- WRONG SHARING FOR OLD MODEL TO HERE
