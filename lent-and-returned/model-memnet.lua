...
function soft_retrieval_standard(query,entity_matrix,output_entity_matrix,temperature)
   ...
   -- the query: we obtain a vector that is a weighted sum of all
   -- the memory vectors in the *output* memory (weights =
   -- similarity profile, such that we will return the most similar
   -- vector(s) to the query) (we get a matrix of such vectors
   -- because of mini-batches)
   local retrieved_entity_vector = nn.View(-1):setNumInputDims(2)(nn.MM(false,false)({query_entity_similarity_profile,output_entity_matrix}))
...
function mm_standard(t_inp_size,v_inp_size,mm_size,inp_seq_cardinality,candidate_cardinality,hops_count,temperature,dropout_p,use_cuda)
   ...
   local retrieved_entity_vector = soft_retrieval_standard(query,entity_matrix,output_entity_matrix,temperature)
...
function mm_one_matrix(t_inp_size,v_inp_size,mm_size,inp_seq_cardinality,candidate_cardinality,hops_count,temperature,dropout_p,use_cuda)
   ...
   for current_hop_stage=1,hops_count do
	 ...
	 query = nn.CAddTable()({mapped_query,retrieved_entity_matrix})
      ...
      -- we now do "soft retrieval" of the output matrix vector that matches the query:
      -- we obtain a vector that is a weighted sum of all the memory
      -- vectors in the output memory matrix (weights = similarity profile, such
   ...
   -- the final prediction is the updated query passed through a
   -- mapping
   prediction = nn.LinearNB(mm_size, mm_size)(query)
   
   -- now we call the return_entity_image function to obtain a softmax
   -- over candidate images
   local output_distribution=return_entity_image(v_inp_size,mm_size,candidate_cardinality,dropout_p,inputs,shareList,prediction)
